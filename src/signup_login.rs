use std::{io, sync::{atomic::{AtomicBool, Ordering}, Arc}, thread};
use std::time;
use figlet_rs::FIGfont;
use indicatif::{ProgressBar, ProgressStyle};
use mongodb::bson::Document;

use crate::{database::{self, connection, get_connection, users::find_user}, models::{Hint, Pattern, UserAccount}};

pub async  fn signup_login(){
        ...
        if choice == 1 {
          ...
          let mut users :Vec<Document> = Vec::new();
            ...
            match  choice {
                ...
                2=>{
                    let additional_info = ||->(&Vec<Document>){
                        (&users)
                    };
                    let account_creation = Arc::new(AtomicBool::new(false));
                    let ac = account_creation.clone();
                    ...
                    match connection_result {
                        Ok(_) =>{
                             ...
                             let user =UserAccount{
                              ...
                              battles:vec![],
                            battles_won:0,
                                hint:Hint{hint:String::new(),pattern_rule:String::new()} ,
                            ...
                            match user_doc {
                                None=>{
                                   database::users::create_user_account(user).await;
                                    account_creation.store(true, Ordering::SeqCst);
                                    spinner_thread.join().unwrap();  
                                },
                                Some(username)=>{
